<template>
    <div v-if="Object.keys(cartCoupon).length !== 0"
        class="flex items-center gap-3 mb-4 h-[50px] rounded-lg shadow-coupon">
        <div
            class="flex items-center gap-2 h-full w-[76px] px-2 ltr:rounded-l-lg rtl:rounded-r-lg flex-shrink-0 text-white bg-[#1AB759]">
        ...
        <div class="w-full ltr:rounded-l-lg rtl:rounded-r-lg relative bg-white">
            ...
            <button @click.prevent="destroyCoupon" type="button"
                class="lab lab-delete-bold text-sm absolute top-1/2 ltr:right-5 rtl:left-5 -translate-y-1/2 text-[#FB4E4E]"></button>
    ...
    <button v-else @click.prevent="couponModalShow"
        ...
        <i class="lab-arrow-right rtl:rotate-180 lab-font-bold lab-font-size-22 text-lg text-[#008BBA]"></i>
    </button>
            ...
            <div class="modal-body">
                <form @submit.prevent="couponChecking" class="flex w-full h-12">
                    <input :class="error ? 'invalid' : ''" v-model="couponProps.form.code" type="text"
                        class="w-full h-full px-3 rtl:rounded-r-lg ltr:rounded-l-lg border border-[#D9DBE9]">
                    <button
                        class="py-3 px-5 w-fit flex-shrink-0 ltr:rounded-r-lg rtl:rounded-l-lg font-medium capitalize text-white bg-[#1AB759]">
                        {{ $t('button.apply') }}
                    </button>
                </form>
                ...
                <div v-if="coupons.length > 0" v-for="coupon in coupons" :key="coupon"
                        ...
                        <h4 class="text-xs">
                            {{
                                $t('message.discount_off', {
                                    discount: coupon.discount_type === taxTypeEnum.PERCENTAGE ? coupon.flat_discount + '%' :
                                        currencyFormat(coupon.flat_discount,
                    ...
                    <button @click.prevent="appCouponButton(coupon)" type="button"
                        class="absolute top-0 ltr:right-0 rtl:left-0 text-xs py-1 px-2 rounded-tr-lg rounded-bl-lg capitalize text-white bg-primary">
                        {{ $t('button.apply') }}
                    </button>
