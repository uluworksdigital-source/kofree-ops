...
use App\Models\Marketplace\MarketplaceOrderLog;
            ...
            DB::transaction(function () use ($request) {
                $this->order = FrontendOrder::create(
                    $request->validated() + [
                        ...
                        'order_datetime'   => date('Y-m-d H:i:s'),
                        'preparation_time' => Settings::group('order_setup')->get('order_setup_food_preparation_time'),
						'source' => $request->device_type == 'kiosk'
    ? OrderSource::KIOSK
    : OrderSource::QR,
...
/**
 * Marketplace Sipariş Kaydı (Yemeksepeti, Trendyol Yemek, Getir)
 *
 * @throws Exception
 */
public function marketplaceOrderStore(MarketplaceOrderRequest $request): object
{
    try {
        DB::transaction(function () use ($request) {

            // 1) Local sipariş oluştur
            $this->order = Order::create([
                'user_id'        => null,
                'status'         => OrderStatus::PENDING,
                'order_datetime' => now(),
            ...
            // 3) Marketplace sipariş log kaydı
